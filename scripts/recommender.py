...
class GameRecommender():
    ...
    def fetch_rec_game(self, requester):
        try:
            ...
            # fetch game
            game_info = self.PG_WRAPPER.get_rows(f"SELECT name, slug, platforms, genres, stores, released FROM gd.games g WHERE (SELECT id FROM gd.v_games vg WHERE {fav_genres_str} AND (INT4(released) BETWEEN {int(fav_year)-5} AND {int(fav_year)+5}) AND FLOAT4(rating) > 4 ORDER BY RANDOM() LIMIT 1) = g.id;")[0]
            game_obj = Game(game_info[0],game_info[1],game_info[2],game_info[3],game_info[4],game_info[5])
